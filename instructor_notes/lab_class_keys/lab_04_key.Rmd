...
mingo_working <- mingo %>%
  select(contains("buyer")) %>%
  unite(buyer_address_complete, buyer_address1:buyer_city, remove = 'F', sep=" ") 
...
mingo_working <- mingo %>%
  mutate(pills_category = if_else(dos_str > 10000, "large shipment", if_else(dos_str > 5000, "medium shipment", "small shipment")))
...
mingo_working <- mingo %>%
  mutate(pills_category = case_when(
    dos_str > 10000 ~ "large shipment",
    dos_str > 5000 ~ "medium shipment",
    TRUE ~ "small shipment")
  )
...
mingo %>%
  group_by(buyer_name) %>%
  summarise(pills_per_pharmacy = n()) %>%

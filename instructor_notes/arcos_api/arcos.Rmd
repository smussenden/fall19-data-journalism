...
aa <- county_raw(state="MD", county="Anne Arundel", key=key)

tracts <- pharm_tracts(state="MD", county="ANNE ARUNDEL", key=key)

pharmacies <- buyer_addresses(state="MD", county="ANNE ARUNDEL", key=key)

aa_x <- aa %>%
  inner_join(tracts, by=c("BUYER_DEA_NO"))

```

```{r}

deaths <- read_tsv("../../major_assignments/data_analysis_project/data/cdc_opioid_drug_overdoses/2006-2012.txt")

pharmacies <- pharm_tracts(key=key)

addresses <- buyer_addresses(key=key)
data <- buyer_details(key=key, state="MD")

data <- combined_buyer_annual(key=key)
buyer_details()
# Load list of total pills by county by year (2006 to 2012)
county_pills_per_year <- summarized_county_annual(key = key) %>%
  clean_names() %>%
  group_by(countyfips, buyer_county, buyer_state) %>%
  summarise(average_pills_per_year = round(mean(dosage_unit), 0)) %>%
  arrange(countyfips)
write_csv(county_pills_per_year, "county_pills_per_year.csv")

# Load county population
county_population_per_year <- county_population(key = key) %>%
  clean_names() %>%
  group_by(countyfips, buyer_county, buyer_state) %>%
  summarise(average_population_per_year = round(mean(population), 0))
write_csv(county_population_per_year, "county_population_per_year.csv")


# Load combined buyer annual purchases
mingo_buyer_per_year <- combined_buyer_annual(county="Mingo", state="WV", key = key) %>%
  clean_names()
write_csv(mingo_buyer_per_year, "mingo_buyer_per_year.csv")

# Load buyer addresses 
mingo_buyer_addresses <- buyer_addresses(county="Mingo", state="WV", key=key) %>%
  clean_names()
write_csv(mingo_buyer_addresses, "mingo_buyer_addresses.csv")

buyer_addresses <- buyer_addresses(key=key) %>%
  clean_names()
write_csv(buyer_addresses, "buyer_addresses.csv")

buyer_annual_by_year <- combined_buyer_annual(key=key) %>%
  clean_names()
write_csv(buyer_annual_by_year, "buyer_annual_by_year.csv")

buyer_totals <- buyer_annual_by_year %>%
  group_by(buyer_dea_no, buyer_bus_act, buyer_county, buyer_state) %>%
  summarise(total_pills=sum(dosage_unit))
write_csv(buyer_totals, "buyer_totals.csv")

join_buyer <- buyer_annual %>%
...
county_pills_per_year_x <- county_pills_per_year %>%
  group_by(countyfips, buyer_county, buyer_state) %>%
  summarise(average_pills_per_year = round(mean(dosage_unit), 0)) %>%
  filter(!is.na(countyfips))

county_population_per_year_x <- county_population_per_year %>%
  group_by(countyfips, buyer_county, buyer_state) %>%
  summarise(average_population_per_year = round(mean(population), 0))

join_check <- county_population_per_year_x %>%
  anti_join(county_pills_per_year_x)

```

```{r}

Also going to work with building dates in lubridate.  and taking out year and then doing totals...

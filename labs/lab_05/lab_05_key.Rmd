...
* Learn how to join together two data sets, and figure out problems. 
* Rearrange columns. 
* Do some additional data cleaning, including working with dates. 
...
3. Rename this file "lab_05_FIRSTNAME_LASTNAME.Rmd".
4. Create a new R project inside of this folder, which will set the working directory in this folder.   

## Load Packages

We're loading two packages today: the Tidyverse, Janitor and a new one, [Lubridate](https://lubridate.tidyverse.org/), which is in the Tidyverse family, but it doesn't load when you load library(tidyverse).

**Task**: In the code block below, load the Tidyverse family of packages, the Janitor package, and the Lubridate package
...
Right now, the date column is a mess.  Clean up with lubridate to convert to valid date we can do stuff to.
...
Look, and see it's a valid datetime

```{r}
glimpse(mingo_x)
```

We can then pull out other components
...
A lot more we can do with dates, and among the most common thing you'll do. 

## Joins

Load two new data sets
...
mingo_working <- mingo %>%
  mutate(pills_category = if_else(dosage_units > 10000, "large shipment", "not large shipment"))
...
Write a codeblock below that groups by the manufacturer name, distributor name and phamracy name and calculates the number of pills for each grouping. Sort it highest to lowest by number of pills.  Leave a comment with the answer: what is the most significant combo?
...
# Actavis Pharma, Inc. to	MIAMI-LUKEN to	TUG VALLEY PHARMACY, LLC
...
mingo_working <- mingo %>%
  ...
  group_by(reporter_name, reporter_dea_no, reporter_address1, reporter_address2, reporter_city, reporter_state, reporter_zip) %>%
  summarise(total_pills = sum(dosage_unit)) %>%
  arrange(desc(total_pills))

# By grouping by all of the information we have about these entities, we can see that they refer to three different facilities.  Whether they're all owned by the same company, that will take some reporting.  You could call the companies!  Or look for SEC filings, if they're public.  Or look on the company website. 

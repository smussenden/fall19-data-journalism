...
TWO KINDS OF JOINS -- one to one and one to many

# Load list of total pills by county by year (2006 to 2012)
county_pills_per_year <- summarized_county_annual(key = key) 
county_pills_per_year <- clean_names(county_pills_per_year)

# Load county population
county_population_per_year <- county_population(key = key)
county_population_per_year <- clean_names(county_population_per_year)

# Load combined buyer annual purchases
buyer_per_year <- combined_buyer_annual(key = key)
buyer_per_year <- clean_names(buyer_per_year)

# Load buyer addresses 
buyer_addresses <- buyer_addresses(key=key)
buyer_addresses <- clean_names(buyer_addresses)
```
